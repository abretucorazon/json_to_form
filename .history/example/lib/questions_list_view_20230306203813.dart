...
class QuestionsListView extends StatefulWidget {
  ...
  static Widget navigateTo(BuildContext context) {
    ...
    final args = route?.settings.arguments as Json?;
    final form = args ?? testForm;
...
class _QuestionsListView extends State<QuestionsListView> {
  ...
  void saveAction(Json? fieldMetaData) {
    print("SAVE ACTION");
    print(fieldMetaData);

    if (fieldMetaData != null) {
      Json fieldSchema = textMetaToSchema(fieldMetaData);
      if (widget.formSchema?['fields'] != null) {
        widget.formSchema?['fields'].add(fieldSchema);
...
*/
        ...
        body: ListView(
          children: (formSchema?['fields'] as List<Json>?)
